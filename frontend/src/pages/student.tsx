...
export default function StudentPage() {
  ...
  useEffect(() => {
    async function fetchTeachers() {
      try {
        const res = await fetch('https://gurushish-10.onrender.com/teacher');
    ...
    async function fetchSessions() {
      ...
      try {
        const res = await fetch(`https://gurushish-10.onrender.com/bookings/student/${studentUsername}`);
        const data = await res.json();
        setSessions(data);
      } catch (err) {
  ...
  const handleViewProfile = async (username: string) => {
    ...
    try {
      const res = await fetch(`https://gurushish-10.onrender.com/teacher-profiles/username/${username}`);
      const data = await res.json();
  ...
  const handleBookNow = async (teacher: Teacher) => {
    ...
    try {
      ...
      const res = await fetch('https://gurushish-10.onrender.com/bookings', {
        method: 'POST',
          ...
          <div className="bg-white p-6 rounded-lg w-full max-w-lg relative overflow-y-auto max-h-[90vh]">
            ...
            <div className="space-y-2">
              ...
              {selectedProfile.degree_certificate_path && (
                <img
                  src={`https://gurushish-10.onrender.com/${selectedProfile.degree_certificate_path.replace(/\\/g, '/')}`}
